...
def smooth_data(rawsong, samp_freq, freq_cutoffs=None, smooth_win=10):
    ...
    squared_song_ax = np.power(filtsong_ax, 2)
    ...
    smooth_ax = np.convolve(squared_song_ax, h)
    offset_ax = round((smooth_ax.shape[-1] - filtsong_ax.shape[-1]) / 2)
    #print("offset: %d" % offset)
    smooth_ax = smooth_ax[offset_ax:filtsong_ax.shape[-1] + offset_ax]
	
	
    return (smooth, smooth_ax)
...
#cutoffs : 1000, 8000
(amp,amp_ax) = smooth_data(rawsong,fs,freq_cutoffs=(1000, 8000))
...
(onsets_ax, offsets_ax) = segment_song(amp_ax,segment_params={'threshold': threshold, 'min_syl_dur': 0.02, 'min_silent_dur': 0.005},samp_freq=fs)
shpe_ax = len(onsets_ax)
onsets_ax = np.round(onsets_ax * fs).astype(int)
offsets_ax = np.round(offsets_ax * fs).astype(int)
...
for i in range(0,shpe_ax):
    plt.axvline(x=onsets_ax[i],color='y')
    plt.axvline(x=offsets_ax[i],color='g')	
